...
void
LimitsSampling (std::string fileName, Ptr<OutputStreamWrapper> stream, Ptr<DynamicQueueLimits> queueLimits, float period)
{
  Simulator::Schedule (Seconds (period), &LimitsSampling, fileName, stream, queueLimits, period);
  *stream->GetStream () << " insert get queue limit" << std::endl;
}

void
PacketsInQueueDiscSampling (std::string fileName, Ptr<OutputStreamWrapper> stream, Ptr<QueueDisc> queueDisc, float period)
{
  Simulator::Schedule (Seconds (period), &PacketsInQueueDiscSampling, fileName, stream, queueDisc, period);
  *stream->GetStream () << queueDisc->GetNPackets () << std::endl;
}
...
{
  std::cout << context << "=" << (double) rtt.GetMicroSeconds () / 1000 << " ms" << std::endl;
}
...
int main (int argc, char *argv[])
{
  ...
  uint32_t netdevicesQueueSize = 256;
  bool bql = false;

  std::string flowsDatarate = "20Mbps";
  uint32_t flowsPacketsSize = 16384;

  float startTime = 0.1; // in s
  float simDuration = 30;
  float samplingPeriod = 1;

  uint8_t pingTos = 0x00;
...
//   Config::SetDefault ("ns3::NetDeviceQueueInterface::MaxPktsBetweenInterrupts", UintegerValue (10));
//   Config::SetDefault ("ns3::NetDeviceQueueInterface::MaxTimeBetweenInterrupts", TimeValue (MilliSeconds (50)));
  ...
  if (queueDiscType.compare ("PfifoFast") == 0)
  ...
  else if (queueDiscType.compare ("ARED") == 0)
    {
      ...
      Config::SetDefault ("ns3::RedQueueDisc::Mode", EnumValue (QueueBase::QUEUE_MODE_PACKETS));
  ...
  else if (queueDiscType.compare ("CoDel") == 0)
    {
      ...
      Config::SetDefault ("ns3::CoDelQueueDisc::Mode", EnumValue (QueueBase::QUEUE_MODE_PACKETS));
  ...
  else if (queueDiscType.compare ("PIE") == 0)
    {
      ...
      Config::SetDefault ("ns3::PieQueueDisc::Mode", EnumValue (QueueBase::QUEUE_MODE_PACKETS));
  ...
  Config::SetDefault ("ns3::QueueBase::MaxPackets", UintegerValue (256));
  ...
  queueInterface->SetNumPackets (10);
  queueInterface->SetAbsoluteTimer (MicroSeconds (300));
  ...
  Ptr<OutputStreamWrapper> streamBytesInQueue = ascii.CreateFileStream (queueDiscType + "-packetsInQueue.txt");
//   qdiscs.Get(0)->TraceConnectWithoutContext ("PacketsInQueue",MakeBoundCallback (&BytesInQueueTrace, streamBytesInQueue));
  Ptr<OutputStreamWrapper> packetsInQueueDiscStream = ascii.CreateFileStream (queueDiscType + "-packetsInQueueDisc.txt");
  Simulator::Schedule (Seconds (samplingPeriod), &PacketsInQueueDiscSampling, queueDiscType + "-packetsInQueueDisc.txt", packetsInQueueDiscStream, qdiscs.Get (0),
                      samplingPeriod);
  ...
  uint16_t port = 7;
  ApplicationContainer uploadApp, sourceApps;
  // Configure and install upload flow


  for (int i = 0; i < 50; i++)
    {
      InetSocketAddress add = InetSocketAddress (Ipv4Address::GetAny (), port + i);
      add.SetTos (0x00);
      Address addUp (add);
      ...
      InetSocketAddress socketAddressUp = InetSocketAddress (n3Interface.GetAddress (0), port + i);
      OnOffHelper onOffHelperUp ("ns3::TcpSocketFactory", Address ());
      onOffHelperUp.SetAttribute ("Remote", AddressValue (socketAddressUp));
      onOffHelperUp.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=0.9]"));
      onOffHelperUp.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0.1]"));
      ...
      sourceApps.Add (onOffHelperUp.Install (n2));
    }
...
//   sourceApps.Add (onOffHelperDown.Install (n3));
  ...
  Config::SetDefault ("ns3::V4Ping::Interval", TimeValue (MilliSeconds (500)));
  Config::SetDefault ("ns3::V4Ping::Tos", UintegerValue (pingTos));
...
//                        downloadGoodputStream, samplingPeriod);
  ...
  flowMon->Enable (1, simDuration, false, "bqlWriter");

  Simulator::Stop (Seconds (stopTime));
  Simulator::Run ();

  flowMon->GetFlowMonitor ()->SerializeToXmlFile("bqlFlowMonitor.xml", true, true);
