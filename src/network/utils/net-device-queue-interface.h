...
public:
   ...
   *
   * Note that this method is invoked before the dequeued packet is actually
   * returned to the device and calls the wake callback when there is room for
   * another packet. If the queue is stopped, the wake callback invokes the
   * queue disc Run method, which sends another packet to the netdevice. This
   * event might trigger the dequeue of another packet, which would be processed
   * before the previous one is returned to the netdevice. In order to avoid
   * that, the netdevice must set the InOrderDequeue attribute of its queues
   * to true.
   */
  template <typename Item>
...
protected:
  ...
  static void NsLog (const enum LogLevel level, Args... args);
...
template <typename... Args>
void
NetDeviceQueue::NsLog (const enum LogLevel level, Args... args)
{
  std::stringstream ss;
                                ...
                                uint8_t txq, Ptr<const Item> item)
{
  ...
  if ((queue->GetMode () == QueueBase::QUEUE_MODE_PACKETS &&
    ...
    {
      NsLog (LOG_DEBUG, "The device queue is being stopped (", queue->GetNPackets (),
                                 ...
                                 uint8_t txq, Ptr<const Item> item)
{
  ...
  NsLog (LOG_ERROR, "BUG! No room in the device queue for the received packet! (",
