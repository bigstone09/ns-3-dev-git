...
protected:
   ...
   * \param args a comma separated list of messages to log
   */
  template <typename... Args>
  void NsLog (const enum LogLevel level, Args... args) const;
 ...
 * hence forward declaration in their header files is recommended to avoid
 * pulling the implementation of the templates included in this file:
 *
 * \code
 *   template <typename Item> class Queue;
 * \endcode
 *
 * This header file is then included in the modules actually requiring the
 * implementation of the class Queue methods.
 */
template <typename Item>
...
template <typename... Args>
void
QueueBase::NsLog (const enum LogLevel level, Args... args) const
{
  std::stringstream ss;
  int dummy[sizeof...(Args)] = { (ss << args, 0)... };
  NS_UNUSED (dummy);
...
{
  NsLog (LOG_DEBUG, "Queue:DoEnqueue(", this, ",", item, ")");

  if (m_mode == QUEUE_MODE_PACKETS && (m_nPackets.Get () >= m_maxPackets))
    {
      NsLog (LOG_LOGIC, "Queue full (at max packets) -- dropping pkt");
  ...
  if (m_mode == QUEUE_MODE_BYTES && (m_nBytes.Get () + item->GetSize () > m_maxBytes))
    {
      NsLog (LOG_LOGIC, "Queue full (packet would exceed max bytes) -- dropping pkt");
      DropBeforeEnqueue (item);
      return false;
    }
  ...
  NsLog (LOG_LOGIC, "m_traceEnqueue (p)");
  m_traceEnqueue (item);
...
{
  NsLog (LOG_DEBUG, "Queue:DoDequeue(", this, ")");

  if (m_nPackets.Get () == 0)
    {
      NsLog (LOG_LOGIC, "Queue empty");
      return 0;
    }
  ...
  if (item != 0)
    {
      ...
      NsLog (LOG_LOGIC, "m_traceDequeue (p)");
      m_traceDequeue (item);
    }
  return item;
}
...
{
  NsLog (LOG_DEBUG, "Queue:DoRemove(", this, ")");
...
{
  NsLog (LOG_DEBUG, "Queue:Flush(", this, ")");
  while (!IsEmpty ())
    {
      Remove ();
    }
...
{
  NsLog (LOG_DEBUG, "Queue:DoPeek(", this, ")");
...
{
  NsLog (LOG_DEBUG, "DropBeforeEnqueue ", this, item);
  ...
  NsLog (LOG_LOGIC, "m_traceDropBeforeEnqueue (p)");
  m_traceDrop (item);
...
{
  NsLog (LOG_DEBUG, "DropAfterDequeue ", this, item);
  ...
  NsLog (LOG_LOGIC, "m_traceDropAfterDequeue (p)");
  m_traceDrop (item);
