 ...
 *
 * Devices aiming to be Traffic Control aware must perform the following
 * operations:
 *   - in DoInitialize ()
 *       + cache the pointer to the netdevice queue interface
 *       + set the number of device transmission queues through the
 *         netdevice queue interface, if the device is multi-queue
 *       + set the select queue callback through the netdevice
 *         queue interface, if the device is multi-queue
