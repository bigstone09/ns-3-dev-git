 ...
 * -----------------------------------             -----------------------------------
 *
 * This example build a node with two interface in emulation mode.
 * The user can explore different qdiscs behaviours on the backlog of a device emulated bottleneck side.
 *
 */
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("index", "Exp index", index);
  ...
  tch0.SetRootQueueDisc ("ns3::PfifoFastQueueDisc");
  ...
  if (queueDiscType.compare ("PfifoFast") == 0)
    ...
    {
      uint32_t handle = tch1.SetRootQueueDisc ("ns3::FqCoDelQueueDisc");
      tch1.AddPacketFilter (handle, "ns3::FqCoDelIpv4PacketFilter");
      tch1.AddPacketFilter (handle, "ns3::FqCoDelIpv6PacketFilter");
    }
